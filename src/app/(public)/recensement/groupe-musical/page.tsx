...
import { X, Check, ChevronRight, ArrowRight, Music2, MapPin, Mic, User } from "lucide-react"
import { cn } from "@/lib/utils"
import { CustomSelect } from "@/components/ui/custom-select"

const steps = [
  { id: 1, name: "Identité" },
  { id: 2, name: "Spécificités Musicales" },
  { id: 3, name: "Vision" }
]

const categories = [
  { value: "lead", label: "Lead (Conducteur)", description: "Voix principale", icon: <Mic className="w-4 h-4" /> },
  { value: "choeur", label: "Chœur", description: "Accompagnement vocal", icon: <User className="w-4 h-4" /> },
  { value: "musicien", label: "Musicien", description: "Instrumentiste uniquement", icon: <Music2 className="w-4 h-4" /> }
]
...
export default function GroupeMusicalFormPage() {
  ...
  const handleSubmit = async () => {
    ...
    data.append("group_type", "GROUPE_MUSICAL")
  ...
  const isStepValid = () => {
    ...
    if (currentStep === 2) {
      return formData.categorie_chantre
    }
    return true
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-slate-50 overflow-hidden font-sans">
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-[20%] right-[10%] w-[30%] h-[30%] rounded-full bg-violet-400/20 blur-[100px]" />
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 overflow-y-auto pb-24">
        ...
        <div className="w-full max-w-2xl bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-2xl shadow-indigo-900/5 border border-white/50 p-8 md:p-12 relative">
          ...
          {currentStep === 1 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div>
                ...
                <p className="text-slate-500">Vos informations pour le registre national.</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-700">Nom *</label>
                  <input type="text" name="nom" value={formData.nom} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="Votre nom" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-700">Prénom *</label>
                  <input type="text" name="prenom" value={formData.prenom} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="Votre prénom" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-700">Téléphone *</label>
                  <input type="tel" name="telephone" value={formData.telephone} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="0700000000" />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" name="profession" value={formData.profession} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="Votre métier" />
                </div>
              </div>
          ...
          {currentStep === 2 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div>
                <h2 className="text-3xl font-bold text-slate-900 mb-2">Spécificités Musicales</h2>
                <p className="text-slate-500">Dites-nous en plus sur vos talents musicaux.</p>
              </div>

              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-700">Catégorie Chantre *</label>
                <div className="max-w-md">
                  <CustomSelect 
                    value={formData.categorie_chantre} 
                    onChange={(val) => handleSelectOption("categorie_chantre", val)} 
                    options={categories}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-700">Instrument(s) joué(s)</label>
                  <input type="text" name="instrument" value={formData.instrument} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="Ex: Piano, Basse, Batterie" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-slate-700">Partition vocale</label>
                  <input type="text" name="partition" value={formData.partition} onChange={handleInputChange} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="Ex: Ténor, Alto..." />
                </div>
              </div>
              ...
              {steps.map((step) => (
                <div 
                  ...
                  className={cn(
                    ...
                    step.id === currentStep ? "w-6 bg-indigo-600" : 
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={cn(
                  ...
                  (!isStepValid() || isSubmitting) ? "bg-indigo-300 cursor-not-allowed" : "bg-indigo-600 hover:bg-indigo-700 hover:shadow-lg hover:-translate-y-0.5"
                )}
              >
                {isSubmitting ? (
